...
export async function writeTopicReport(report) {
  await fs.ensureDir(REPORTS_DIR);

  const latest = path.join(REPORTS_DIR, "latest-run.json");
  const dated = path.join(REPORTS_DIR, `topic-run-${timestampForFilename()}.json`);
...
export async function readLatestTopicReport() {
  const latest = path.join(REPORTS_DIR, "latest-run.json");
